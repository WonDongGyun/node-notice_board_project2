...
// beforeAll(async () => {
//     // sequelize.accountData.create = jest.fn(() => Promise.resolve());
//     const nickname = 'helloTest123';
//     const password = '123456';

//     await sequelize.sync();
//     const user = await Account.create({ nickname: nickname, password: password });
//     console.log(user);
// });
...
test("닉네임은 최소 3자 이상이며, 알파벳 대소문자(a~z, A~Z) 및 숫자(0~9)의 혼합으로 이루어져야 합니다.", async (done) => {
    ...
    expect(res.status).toEqual(400);
    expect(JSON.parse(res.text)['modal_body']).toEqual('닉네임은 최소 3자 이상이며, 알파벳 대소문자(a~z, A~Z) 및 숫자(0~9)의 혼합으로 이루어져야 합니다.');
    done();
});
...
test("비밀번호는 최소 4자 이상이며, 닉네임과 같은 값이 포함된 경우 회원가입에 실패합니다.", async (done) => {
    ...
    expect(res.status).toEqual(400);
    expect(JSON.parse(res.text)['modal_body']).toEqual('비밀번호는 최소 4자 이상이며, 비밀번호에 닉네임과 같은 값이 포함되면 안됩니다.');
    done();
});
...
test("비밀번호 확인은 비밀번호와 정확하게 일치해야 합니다.", async (done) => {
    ...
    expect(res.status).toEqual(400);
    expect(JSON.parse(res.text)['modal_body']).toEqual('비밀번호 확인은 비밀번호와 정확하게 일치해야 합니다.');
    done();
});
...
describe('addFollowing', () => {
    ...
    test("데이터베이스에 존재하는 닉네임을 입력한 채 회원가입 버튼을 누른 경우 '중복된 닉네임입니다.' 라는 에러메세지가 발생합니다.", async (done) => {
        ...
        expect(JSON.parse(res.text)['modal_body']).toEqual('회원이 되신것을 축하드립니다!');
        done();
    });
